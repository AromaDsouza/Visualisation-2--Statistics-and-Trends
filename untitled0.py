# -*- coding: utf-8 -*-
"""
Created on Thu Jan 26 14:58:40 2023

@author: Aroma
"""

import pandas as pd  #Importing the pandas library for the analysis of the data
import matplotlib.pyplot as plt  #Importing the matplotlib library for visualization of the data
import numpy as np  #Importing the numpy library for the mathematical operations on the arrays

def population_forest(population_forest_data):
    
    '''
    The above function named population produces a grouped bar graph 
    i.e side to side bar graph of the dataset from World Bank data.
    population_forest_data: It is the dataset used for plotting the bar graph
    '''
    
    #To read the data and create a dataframe
    dataframe1 = pd.read_csv(population_forest_data, skiprows=4)  
...
a,b = population_forest("Population.csv")
...
c,d = population_forest("Forest Area in sq km.csv")
...
def c02_emissions_renewable_energy(c02_emissions_renewable_energy_data):
    '''
    The above function named c02_emissions_renewable_energy produces a pie chart 
    of the dataset from World Bank data.
    forest_area_data: It is the dataset used for plotting the pie chart.
    '''
    
    #To read the data and create a dataframe
    dataframe1 = pd.read_csv(c02_emissions_renewable_energy_data, skiprows=4)
    #To select the desired rows and columns respectively
    dataframe1 = dataframe1.iloc[[35, 40, 77, 81, 109, 113, 119, 126],
                                 [0, 40, 50, 60]]
    #To drop the NaN or empty cells in the columns
...
e,f = c02_emissions_renewable_energy("CO2 Emissions.csv")
...
g,h = c02_emissions_renewable_energy("Renewable energy.csv")
...
def climate_change(climate_change_data):
    ...
    filter_1 = dataframe1['Indicator Name'].isin(indicator)
    dataframe2 = dataframe1.loc[filter_1]
